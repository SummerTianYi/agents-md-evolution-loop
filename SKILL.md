...
8. Create a Codex scheduled task using the generated `<instance-root>/automation-prompt.md`. It must run in a Gmail-capable Codex environment using the lightest visible local model at the instance `routine_check_reasoning_effort` (default `low`); it is a dispatch task and must not use its own model to analyze model releases or draft reports. Only `run_loop.py`, after a detected change, launches the highest-priority visible model at `max` for the author and independent reviewer. Only this task may send and verify the report in Sent. After creation succeeds, run `python scripts/record_automation.py --root <instance-root> --automation-id <automation-id>` so the instance manifest records the active task.
